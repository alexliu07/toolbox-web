...
// 获取点赞状态
router.get('/like/status', optionalAuth, async (req, res) => {
  try {
    const { bvid } = req.query
    if (!bvid) return res.status(400).json({ code: -400, message: 'bvid is required' })
    const cookieStr = getCookiesString(req.user?.id)
    if (!cookieStr) return res.json({ code: -101, message: '账号未登录' })
    const data = await fetchUrl('https://api.bilibili.com/x/web-interface/archive/has/like', { bvid }, { cookies: cookieStr })
    res.json(data)
  } catch (e) {
    console.error('Like status error:', e.message)
    res.status(500).json({ code: -500, message: e.message })
  }
})

// 点赞/取消点赞
router.post('/like', optionalAuth, async (req, res) => {
  try {
    const { bvid, like = 1 } = req.body
    if (!bvid) return res.status(400).json({ code: -400, message: 'bvid is required' })
    const cookieStr = getCookiesString(req.user?.id)
    if (!cookieStr) return res.json({ code: -101, message: '账号未登录' })
    const cred = getBilibiliCredentials(req.user?.id)
    const csrf = cred?.cookies?.bili_jct || ''
    const params = new URLSearchParams({ bvid, like: String(like), csrf })
    const data = await fetchUrl('https://api.bilibili.com/x/web-interface/archive/like', {}, {
      method: 'POST', cookies: cookieStr,
...
// 获取投币状态
router.get('/coin/status', optionalAuth, async (req, res) => {
  try {
    const { bvid } = req.query
    if (!bvid) return res.status(400).json({ code: -400, message: 'bvid is required' })
    const cookieStr = getCookiesString(req.user?.id)
    if (!cookieStr) return res.json({ code: -101, message: '账号未登录' })
    const data = await fetchUrl('https://api.bilibili.com/x/web-interface/archive/coins', { bvid }, { cookies: cookieStr })
    res.json(data)
  } catch (e) {
    console.error('Coin status error:', e.message)
    res.status(500).json({ code: -500, message: e.message })
  }
})

// 投币
router.post('/coin', optionalAuth, async (req, res) => {
  try {
    const { bvid, multiply = 1, select_like = 0 } = req.body
    if (!bvid) return res.status(400).json({ code: -400, message: 'bvid is required' })
    const cookieStr = getCookiesString(req.user?.id)
    if (!cookieStr) return res.json({ code: -101, message: '账号未登录' })
    const cred = getBilibiliCredentials(req.user?.id)
    const csrf = cred?.cookies?.bili_jct || ''
    const params = new URLSearchParams({ bvid, multiply: String(multiply), select_like: String(select_like), csrf })
    const data = await fetchUrl('https://api.bilibili.com/x/web-interface/coin/add', {}, {
      method: 'POST', cookies: cookieStr,
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: params.toString()
    })
